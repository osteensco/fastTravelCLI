...
// ExpandAndCleanPath converts a path, expanding environment variables,
// resolving '~' to the user's home directory, and cleaning the path
// to its absolute, canonical form.
func ExpandAndCleanPath(p string) (string, error) {
	// First, expand any environment variables. This is done first so that
	// a variable like `$MY_PATH` can expand to `~/some/dir` and the tilde
	// will be correctly processed in the next step.
	p = os.ExpandEnv(p)

	// Second, handle tilde expansion for the home directory.
	...
	// Third, resolve the path to an absolute path.
	absPath, err := filepath.Abs(p)
	if err != nil {
		return "", fmt.Errorf("failed to get absolute path for '%s': %w", p, err)
	}

	// Finally, clean the path to resolve any "." or ".." components.
